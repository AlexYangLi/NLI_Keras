...
class KerasInfersentModel(KerasBaseModel):
    def __init__(self, config):
        super(KerasBaseModel, self).__init__(config)
    ...
    def build(self):
        ...
        premise_encoded, hypothesis_encoded = self.sentence_encoder(premise_embed, hypothesis_embed, 'bilstm_max_pool')
        ...
        return Model([input_premise, input_hypothesis], output)
    ...
    def sentence_encoder(self, premise, hypothesis, encoder_type):
        if encoder_type == 'lstm':
            ...
            return GlobalMaxPooling1D()(bilstm(premise)), GlobalMaxPooling1D()(bilstm(hypothesis))
            ...
            return concatenate(attend_premise, attend_hypothesis)
            ...
            cnn_premise = [GlobalMaxPooling1D(cnn_premise[i]) for i in range(1, 5)]
            cnn_hypothesis = [GlobalMaxPooling1D(cnn_hypothesis[i]) for i in range(1, 5)]
